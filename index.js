...
assert.deepEqual({ books: new Set().add('Война и мир, Voïna i mir'), customers: new Set()}, store.getState())
store.dispatch({ type: 'ADD_BOOK', title: 'Война и мир, Voïna i mir' })
assert.deepEqual({ books: new Set().add('Война и мир, Voïna i mir'), customers: new Set()}, store.getState())
store.dispatch({ type: 'REMOVE_BOOK', title: 'Война и мир, Voïna i mir' })
assert.deepEqual( { books: new Set(), customers: new Set()}, store.getState())
store.dispatch({ type: 'ADD_CUSTOMER', customer: 'Tintin' })
assert.deepEqual({ books: new Set(), customers: new Set().add('Tintin')}, store.getState())
